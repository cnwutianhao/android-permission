...
internal class RequestBackgroundLocationPermission internal constructor(permissionBuilder: PermissionBuilder)
    ...
    override fun request() {
        if (pb.shouldRequestBackgroundLocationPermission()) {
            ...
            if (RuntimePerms.isGranted(pb.activity, ACCESS_BACKGROUND_LOCATION)) {
                // ACCESS_BACKGROUND_LOCATION has already granted, we can finish this task now.
                finish()
                return
            }
            val accessFindLocationGranted = RuntimePerms.isGranted(pb.activity, Manifest.permission.ACCESS_FINE_LOCATION)
            val accessCoarseLocationGranted = RuntimePerms.isGranted(pb.activity, Manifest.permission.ACCESS_COARSE_LOCATION)
