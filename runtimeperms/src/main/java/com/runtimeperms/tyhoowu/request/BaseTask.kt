    ...
    override fun finish() {
        ...
        next?.request() ?: run {
            ...
            if (pb.shouldRequestBackgroundLocationPermission()) {
                if (RuntimePerms.isGranted(pb.activity, RequestBackgroundLocationPermission.ACCESS_BACKGROUND_LOCATION)) {
            ...
            if (pb.shouldRequestNotificationPermission()) {
                if (RuntimePerms.areNotificationsEnabled(pb.activity)) {
                    pb.grantedPermissions.add(RuntimePerms.POST_NOTIFICATIONS)
                } else {
                    deniedList.add(RuntimePerms.POST_NOTIFICATIONS)
                }
            }
            if (pb.shouldRequestBodySensorsBackgroundPermission()) {
                if (RuntimePerms.isGranted(pb.activity, RequestBodySensorsBackgroundPermission.BODY_SENSORS_BACKGROUND)) {
